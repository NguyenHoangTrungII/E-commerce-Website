...
if (isset($_POST['cash_on_delivery'])) {
    ...
    $columnName = $tableName = $whereValue = null;
    $tableName = "donhang";
    $columnName['id'] = $lastID[0]['id'] +1;
    $columnName['id_nguoidung'] = $_SESSION['SSCF_login_id'];
    $columnName['giatri'] =$_POST['total-hidden'];
    $columnName['tongtienhang'] =(int) $_POST['total-producPrice-hidden'] -(int) $_POST['discount-hidden'];
    $columnName['tienship'] =$_POST['shipfee-hidden'];
    $columnName['ngaydat'] = date("Y/m/d");
    $columnName['pt_thanhtoan'] = 1;
    ...
    $columnName['id_donhang'] = $lastID[0]['id'] +1;
    $columnName['id_nguoidung'] = $_SESSION['SSCF_login_id'];
    ...
    echo 1;
    header("Location: http://localhost/DoAnWeb/DoAnWeb_testMVC/View/invoice.php?pt=thanhtoansau&donhang= ".($lastID[0]['id'] +1)."");
}
else{
    // var_dump("cc");
}
